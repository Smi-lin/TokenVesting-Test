...
async function main() {
  ...
  const celoTokenInstance = await hre.ethers.getContractAt(
    "CeloToken",
    celoTokenTx
  );
  ...
  const mintNairaToContract = await celoTokenInstance
    .connect(owner)
    .mint(tokenVestingInstance.getAddress(), amountMintToContct);
  ...
  const user1CeloBal = await celoTokenInstance
    .connect(user1)
  ...
  console.log(
    "##### getting user1 releasable amount after 1min of vesting period  #####"
  );

  await time.increaseTo((await time.latest()) + 60);

  const user1ReleasableAmount = await tokenVestingInstance
    .connect(user1)
    .getReleasableAmount(user1.address);
  ...
  const user1CeloBalAfterClim1 = await celoTokenInstance
    .connect(user1)
    .balanceOf(user1.address);

  console.log({
    "user1 balance after  first claiming": user1CeloBalAfterClim1.toString(),
  });

  console.log(
    "### user1 claiming all the the available allocation in time equal or above vesting period ####"
  );
  ...
  const user1CeloBalAfterClaimAll = await celoTokenInstance
    .connect(user1)
    .balanceOf(user1.address);

  console.log({
    "user1 balance after claiming All": user1CeloBalAfterClaimAll.toString(),
  });
}
